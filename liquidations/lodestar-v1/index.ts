...
const EXPLORER_BASE_URL = "https://arbiscan.io/address/";
...
const positions = async () => {
    ...
    const prices = await getUnderlyingPrices(markets, "arbitrum:");

    // all positions across all users
    const positions = accounts.flatMap((account) => {
        const _totalBorrowValueInUsd = totalBorrowValueInUsd(account, prices, "arbitrum:");
        const _totalCollateralValueInUsd = totalCollateralValueInUsd(account, prices, "arbitrum:");
        ...
        const debts = account.tokens
            ...
            .map((token) => {
                const _borrowBalanceUnderlying = borrowBalanceUnderlying(token);
                const _supplyBalanceUnderlying = supplyBalanceUnderlying(token);
                const _price = prices["arbitrum:" + token.market.underlyingAddress];
                if (!_price) {
                    // console.log("no price for", "arbitrum:" + token.market.underlyingAddress);
                    return {
                        debt: new BigNumber(0),
                        price: 0,
                        token: "arbitrum:" + token.market.underlyingAddress,
